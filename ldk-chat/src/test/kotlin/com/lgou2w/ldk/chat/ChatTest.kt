...
class ChatTest {
  ...
  @Test fun `Chat - toColor`() {
    val raw = "&aGreen"
    val target = "§aGreen"
    raw.toColor() shouldBeEqualTo target
    arrayOf(raw).toColor()[0] shouldBeEqualTo target
    listOf(raw).toColor().first() shouldBeEqualTo target
    "&Green".toColor() shouldBeEqualTo "&Green"
  }

  @Test fun `Chat - stripColor`() {
    val raw = "§aGreen"
    val target = "Green"
    raw.stripColor() shouldBeEqualTo target
    arrayOf(raw).stripColor()[0] shouldBeEqualTo target
    listOf(raw).stripColor().first() shouldBeEqualTo target
  }
  ...
  @Test fun `Chat - toComponent`() {
    ...
    component.extraSize shouldBeEqualTo 1
    component.extras.first().style.color shouldBeEqualTo ChatColor.GREEN
    (component.extras.first() as ChatComponentText).text shouldBeEqualTo "Green"
    component.toRaw() shouldBeEqualTo "§aGreen"
    component.toRaw(false) shouldBeEqualTo "Green"
  }

  @Test fun `Chat - toComponentOrNull`() {
    var str : String? = null
    str.toComponentOrNull() shouldBeEqualTo null
    str = "Text"
    str.toComponentOrNull()?.toRaw() shouldBeEqualTo "Text"
  }

  @Test fun `ChatEvent - test`() {
    val cce = ChatClickEvent(ChatClickEvent.Action.OPEN_URL, "github.com")
    cce.action shouldBeEqualTo ChatClickEvent.Action.OPEN_URL
    cce.value shouldBeEqualTo "github.com"
    val che = ChatHoverEvent(ChatHoverEvent.Action.SHOW_TEXT, ChatComponentText("hi"))
    che.action shouldBeEqualTo ChatHoverEvent.Action.SHOW_TEXT
    che.value shouldBeEqualTo ChatComponentText("hi")
  }

  @Suppress("ReplaceCallWithBinaryOperator")
  @Test fun `ChatComponent - NULL`() {
    ChatComponent.NULL shouldNotBeEqualTo null
    ChatComponent.NULL shouldBeInstanceOf ChatComponent::class
    ChatComponent.NULL shouldBe ChatComponent.NULL
    ChatComponent.NULL.hashCode() shouldBeEqualTo 0
    ChatComponent.NULL.extraSize shouldBeEqualTo 0
    ChatComponent.NULL.equals(ChatComponent.NULL) shouldBeEqualTo true
    ChatComponent.NULL.equals(ChatComponentText()) shouldBeEqualTo false
